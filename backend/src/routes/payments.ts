...
router.post("/create-order", async (req: Request, res: Response) => {
  try {
    ...
    const { success } = rateLimit(ip as string, { maxRequests: 10, windowMs: 60000 });
    if (!success) {
...
router.post("/webhook", async (req: Request, res: Response) => {
  try {
    ...
    let payment = null;
    if (razorpayOrderId) {
      payment = await prisma.payment.findUnique({ where: { razorpayOrderId } });
    } else if (razorpayPaymentId) {
      payment = await prisma.payment.findFirst({ where: { paymentId: razorpayPaymentId } });
    }
